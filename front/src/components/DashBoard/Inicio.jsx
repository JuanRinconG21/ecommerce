...
const Inicio = () => {
  ...
  //FUNCION DEL FECTH
  const Listar = async () => {
    const request = await fetch("http://localhost:2100/usuarios/listar", {
      method: "GET",
      headers: {
        Authorization: `${token}`,
      },
  ...
  const Eliminar = (id, nombre) => {
    MySwal.fire({
      title: `¿ Quieres Eliminar el Cliente ${nombre} ?`,
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:2100/usuarios/eliminar/${id}`, {
